...
external_stylesheets = [
    '/home/christan/Desktop/Covid Dashboard (2)/assets/style.css']
...
def recovered(view):
    ...
    return {
        ...
        'layout': go.Layout(
            ...
            font=dict(color=dash_colors['green']),
            font_family='Handlee',
            paper_bgcolor=dash_colors['background'],
            plot_bgcolor=dash_colors['grid'],
            height=200
...
def worldwide_trend(view, population):
    ...
    return {
        ...
        'layout': go.Layout(
            ...
            paper_bgcolor=dash_colors['background'],
            plot_bgcolor=dash_colors['grid'],
            xaxis=dict(gridcolor=dash_colors['background']),
            yaxis=dict(gridcolor=dash_colors['background'])
        )
    }
...
def set_countries_value(view, available_options):
    ...
    '''
    if view == 'Worldwide':
        return ['US', 'Italy', 'United Kingdom', 'Spain', 'France', 'Russia', 'Brazil', 'Sweden', 'Belgium']
    elif view == 'United States':
        return ['New York', 'New Jersey', 'Massachusetts', 'Pennsylvania', 'California', 'Michigan', 'Washington',
                'Illinois']
    elif view == 'Europe':
        ...
        return ['US', 'Italy', 'United Kingdom', 'Spain', 'India', 'Germany', 'Russia']
...
@app.callback(
    Output('India_trend', 'figure'),
    [Input('global_format', 'value')])
def India_trend(view):
    '''
    creates graph for Indian covid-19 data
    
    '''
    
    df_india = pd.read_csv('states_data.csv')
    
    
    confirmed = df_india.groupby('Date')['confirmed'].sum()
    active = df_india.groupby('Date')['active'].sum()
    recovered = df_india.groupby('Date')['recovered'].sum()
    deaths = df_india.groupby('Date')['deaths'].sum()
    title_suffix = ''
    hover = '%{y:,g}'

    traces = [go.Scatter(
                    x=df_india.groupby('Date')['Date'].first(),
                    y=confirmed,
                    hovertemplate=hover,
                    name="confirmed",
                    mode='lines'),
    ...
    return {
            'data': traces,
            'layout': go.Layout(
                title="{} {}".format("Covid-19 cases Trend in India", title_suffix),
                xaxis_title="Date",
                yaxis_title="Number of Cases",
                font=dict(color=dash_colors['text']),
                paper_bgcolor=dash_colors['background'],
...
app.layout = html.Div(style={'backgroundColor': dash_colors['background']}, children=[
    ...
    html.Div(  # worldwide_trend and active_countries
        [
            html.Div(
                ...
                style={'width': '45%', 'float': 'left', 'display': 'center',
            ...
            html.Div([
                ...
                html.Div([
                    ...
                    dcc.Dropdown(
                        id='country_select',
                        multi=True,
                        style={'width': '95%', 'float': 'center','background-color':dash_colors['background']}
                    )],
                    className="custom-dropdown",
                    style={'width': '100%', 'float': 'center', 'display': 'inline-block'})
            ],
                style={'width': '45%', 'float': 'right', 'vertical-align': 'bottom','margin':'50px 20px 50px 20px',
                    'border': '1px solid white'}
            )],
    ...
    html.Div([dcc.Graph(id='India_trend'),html.A('For visualization of states data in india..Please visit this link', href='https://covid19indiaa.000webhostapp.com/statewise.php', target='_blank')],
        style={'width': '95%',
            'float': 'center',
            'display': 'inline-block',
            'font-family' : 'Marck Script',
